...
function etaPRIME_acc = compute_force_etaPRIME_coulombInteraction(etaPRIME_acc, num_particles, ...
    A, Z, reduced_planck_constant, mass, ...
    gamma, q_pos,...
    simulation_step)

    for particle_itr = 1:num_particles
       % compute eta' at t using equation (34) 
%       etaPRIME_acc(particle_itr, simulation_step) = ((9 * reduced_planck_constant * reduced_planck_constant) / ((4 * mass * mass * gamma(particle_itr, simulation_step).^3))) - ...
%           ((Z*exp(2)/(gamma(particle_itr, simulation_step).^2)) * ...
%           sqrt(6/pi) * ...
%          exp((-3*(q_pos(particle_itr, simulation_step).^2)) / ...
